...
####----------------Decadal Scale Analysis------------------#######


Decadal_df1<-Yearly_dataframe[c(1:10), ]    ####1977-1986#####
summary(Decadal_df1)


#      Annual Precipitation Annual Stream flows   Annual PET  
 
#        Mean   :1313         Mean   :19.441     Mean   :1389  

Decadal_df2<-Yearly_dataframe[c(11:20), ]    ####1987-1996####
summary(Decadal_df2)

#      Annual Precipitation Annual Stream flows   Annual PET 

      # Mean   :1308.0       Mean   :21.575     Mean   :1393

Decadal_df3<-Yearly_dataframe[c(21:30), ]    ####1997-2006####
summary(Decadal_df3)

#    Annual Precipitation Annual Stream flows   Annual PET 
     #Mean   :1201.7       Mean   :22.15      Mean   :1396 

Decadal_df4<-Yearly_dataframe[c(31:41), ]    ####2007-2017####
summary(Decadal_df4)

#      Annual Precipitation Annual Stream flows   Annual PET
#      Mean   :1106          Mean   :13.659        Mean   :1411



P_decadal <- c(1313,1308,1201.7,1106)

Q_decadal <- c(19.441,21.575,22.15,13.659)

E_decadal <- c(1389,1393,1396,1411)

Decade <- c(1, 2, 3, 4)



Decadal <- cbind(Decade, P_decadal,Q_decadal,E_decadal)
Decadal <- as.data.frame(Decadal)
par(mfrow=c(3,1)) 


#####Decadal Trend of Precipitation###

Decade_vs_DecadeP <- ggplot(data=Decadal, mapping=aes(x=Decade,y=P_decadal)) +
  geom_point()+
  geom_line(colour="blue")+
  geom_smooth(method=lm, formula = y ~ x, se=FALSE)
...
print(Decade_vs_DecadeP+ggtitle(" Decadal Trend of Precipitation")
      +labs( x="Decade", y="Average Precipitation "))


#####Decadal Trend of Evapotranspiration ###

Decade_vs_DecadeE <- ggplot(data=Decadal, mapping=aes(x=Decade,y=E_decadal)) +
  geom_point()+
  geom_line(colour="red")+
  geom_smooth(method=lm, formula = y ~ x, se=FALSE)
...
print(Decade_vs_DecadeE+ggtitle(" Decadal Trend of Evapotranspiration")
      +labs( x="Decade", y="Evapotranspiration "))
...
###### Checking for Appropriate Distribution of Daily Precipitation & Daily Flow Data #####

###### Probability Plot Correlation Coefficient Test #####
shapiro.test(df_1$Q)

qqnorm(df_1$P, ylab = "Daily Precipitation")

qqplot(df_1$P,df_1$Q, xlab = " Daily Precipitation", ylab = "Daily Streamflows", 
       main="Q-Q Plot of Precipitation vs Streamflows", pch = 19, col = "lightblue")

# Pearson correlation

text(paste("Correlation:", round(cor(df_1$P, df_1$Q), 2)), x = 25, y = 95)

#### sample size must be between 3 and 5000 & Sample Size of the Dataset is 14975 ####

install.packages(ppcc)

library(ppcc)

ppccTest(df_1$Q,"qnorm")            #ppcc = 0.46395#

ppccTest(df_1$P,"qnorm")            #ppcc = 0.66177#

#### alternative hypothesis: df_1$Q differs from a Normal distribution ###

ppccTest(df_1$Q,"qlnorm")           #ppcc = 0.46395

ppccTest(df_1$P,"qlnorm")           

#### alternative hypothesis: df_1$Q differs from a Log Normal distribution ###

ppccTest(df_1$Q,"qunif")

####alternative hypothesis: df_1$Q differs from a Uniform distribution ####

ppccTest(df_1$Q,"qexp")

####alternative hypothesis: df_1$Q differs from a Exponential distribution ####
##########-------------------------------------#####################################
####simple linear regression for Annual maximum dis. vs Annual max. prec 
model<- lm(Annual_maximum_Q~Annual_maximum_P)
model
library(lessR)
Regression(Annual_maximum_Q~Annual_maximum_P,data = Annual_maximum_P_Q)
summary(model)
plot(model)
confint(model,level = 0.95)
##########daily
model_P_Q<-lm(Manot$Q~Manot$P)
summary(model_P_Q)
confint(model_P_Q,level = 0.95)
Plot(model_P_Q)
#######
model_E_Q<-lm(Manot$Q~Manot$E)
summary(model_E_Q)
Model_P_E<-lm(Manot$E~Manot$P)
summary(Model_P_E)

reg_brief(Manot$E~Manot$P)
##############
...
pairs(Manot[,2:4])
round(cor(Manot[,2:4],method = "pearson"),3)
###########significance of correlation 
library(Hmisc)
multi_linear<- lm(Q~P+E,data = Manot)
confint(multi_linear,level = 0.95)
summary(multi_linear)
plot(multi_linear)
######### Q= 20.47P-4.45E+40.24
multi_linear_P<- lm(P~E+Q,data = Manot)
plot(multi_linear_P)
confint(multi_linear_P,level = 0.95)
summary(multi_linear_P)
##########
multi_linear_E<-lm(E~P+Q,data = Manot)
summary(multi_linear_E)
Regression(E~P+Q,data = Manot)
a<-cbind(Allseason_E,Allseason_P,Allseason_Q)
############seasonal multiliear_DISCHARGE
#########summer
summer_multi_linear_Q<- lm(Summer_Q~(Summer_P+Summer_E),data = a)
summary(summer_multi_linear_Q)
Regression(Summer_Q~(Summer_P+Summer_E),data = a)
#########moonsoon
Regression(Seasonal_Q$Q~Seasonal_P$P+Seasonal_E$E)

##########postmoon
Regression(Post_Monsoon_Q~Post_Monsoon_P+Post_Monsoon_E,data = a)
summary(lm(Post_Monsoon_Q~Post_Monsoon_P+Post_Monsoon_E,data = a))
#########winter
Regression(Winter_Q~Winter_P+Winter_E,data = a)
summary(lm(Winter_Q~Winter_P+Winter_E,data = a)) 
